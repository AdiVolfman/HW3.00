...
bool testDeleteFirstElementOfEmpty() {
    ...
    try {
        ...
        list.remove(
                itStart); // This should not throw an exception but should handle gracefully
        // No exception expected, so no assert here
    }
    catch (...) {
        return false; // If any exception is thrown, the test should fail
...
bool testsBumpPriorityByTypeRange() {
    ...
    manager.assignTask("Bob", Task(200, TaskType::Maintenance,
    ...
    cout << "After bumping Maintenance by 10 (should already be at 100):"
         << endl;
    manager.bumpPriorityByType(TaskType::Maintenance, 10);
    manager.printAllTasks();
    cout << endl;
...
bool testOperatorChaining() {
    ...
    const auto &result1 = list.filter(
            [](const string &s) { return s[0] == 'z'; });
    printList(result1);
...
bool testCompleteTask() {
    ...
    manager.assignTask("Alice",
                       Task(1, TaskType::Development,
                            "Task 1")); // Add Alice to the system
    ...
    manager.assignTask("Bob",
                       Task(4, TaskType::Presentation, "Medium Priority"));
...
bool testTaskManagerPrintMethods() {
    ...
    manager.assignTask("Alice",
                       Task(3, TaskType::Research, "Alice's Development"));
    cout << "printAllEmployees:" << endl;
    manager.printAllEmployees();
    cout << "printAllTasks:" << endl;
    manager.printAllTasks();
    ...
    manager.assignTask("Charlie", Task(4, TaskType::Presentation,
